...
async function requestCredential(url, email, mediation, headers, signal) {
  const requestUrl = new URL(url, window.location.origin);
  requestUrl.searchParams.set("username", email ?? "");

  const optionsResponse = await fetchWithErrorHandling(requestUrl.toString(), {
    method: "POST",
    headers,
    signal,
  });
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    static fallbackFieldAttribute = "data-passkey-fallback-field";
    ...
    connectedCallback() {
      ...
      this.attrs = {
        ...
        creationOptionsUrl:
          this.getAttribute("creation-options-url") ??
          "/Identity/Account/PasskeyCreationOptions",
        requestOptionsUrl:
          this.getAttribute("request-options-url") ??
          "/Identity/Account/PasskeyRequestOptions",
      };

      this.passkeySubmitButton = this.previousElementSibling;

      if (this.passkeySubmitButton?.name === "__passkeySubmit") {
        this.passkeySubmitButton.addEventListener("click", () => {
          this.lastPasskeySubmitIntent = true;
        });
      }

      this.internals.form.addEventListener("submit", (event) => {
        const submitter = event.submitter || document.activeElement;
        const isPasskeySubmit =
          submitter?.name === "__passkeySubmit" ||
          submitter?.value === "__passkeySubmit" ||
          this.lastPasskeySubmitIntent === true;

        if (isPasskeySubmit) {
          event.preventDefault();
          this.lastPasskeySubmitIntent = false;
      ...
      try {
        ...
        try {
          ...
          credentialJson = JSON.stringify({
            ...
            response: {
              attestationObject: this.convertToBase64(
                credential.response.attestationObject,
              ),
              authenticatorData: this.convertToBase64(
                credential.response.authenticatorData ??
                  credential.response.getAuthenticatorData?.() ??
                  undefined,
              ),
              clientDataJSON: this.convertToBase64(
                credential.response.clientDataJSON,
              ),
              publicKey: this.convertToBase64(
        ...
        this.appendFallbackField(
          `${this.attrs.name}.CredentialJson`,
          credentialJson,
        );
        ...
        const errorMessage =
          error.name === "NotAllowedError"
            ...
            : error.message;
        this.appendFallbackField(`${this.attrs.name}.Error`, errorMessage);
        formData.append(`${this.attrs.name}.Error`, errorMessage);
      }
      // Preserve passkey-submit intent for programmatic form submission (some browsers omit submitter metadata).
      this.appendFallbackField("__passkeySubmit", "1");
      this.appendFallbackField("button", "__passkeySubmit");
      formData.append("__passkeySubmit", "1");
    ...
    clearFallbackFields() {
      ...
      const fallbackFields = form.querySelectorAll(
        `input[${this.constructor.fallbackFieldAttribute}="1"]`,
      );
    ...
    appendFallbackField(name, value) {
      const form = this.internals?.form;
      if (!form) {
        return;
      }
      ...
      input.name = name;
      input.value = value ?? "";
      input.setAttribute(this.constructor.fallbackFieldAttribute, "1");
